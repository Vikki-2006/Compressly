...
class AnalyticsService:
    ...
    def compute_dashboard_analytics(self) -> dict:
        ...
        total_saved_gb = round(total_saved_bytes / (1024**3), 3) if total_saved_bytes > 0 else 0.0
        
        # Average savings ratio calculation
        avg_savings_pct = 0.0
        if completed_logs:
            pcts = [log.saved_percentage for log in completed_logs if log.saved_percentage is not None]

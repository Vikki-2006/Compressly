...
print(
    f"[PASS] 3. Metadata Extraction OK ({video_meta['width']}x{video_meta['height']}, {video_meta['video_codec']})"
)
...
for _ in range(30):
    ...
    print(
        f"   Polling Status: {status_data['status']} ({status_data.get('progress', 0)}%)"
    )
...
assert (
    "ffmpeg" in log_data["log"].lower() or "command" in log_data["log"].lower()
), "Log text missing FFmpeg command"
...
assert (
    target_row["preset_used"] == "whatsapp"
), f"Preset mismatch: {target_row['preset_used']}"
assert (
    target_row["video_codec"] == "h264"
), f"Codec mismatch: {target_row['video_codec']}"
assert "saved_mb" in target_row, "Saved MB key missing"
print(
    f"[PASS] 9. SQLite History Verification OK (Saved: {target_row['saved_mb']} MB, Pct: {target_row['saved_percentage']}%)"
)

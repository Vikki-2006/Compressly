...
TEST_VIDEO_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scratch_test_sample.mp4")
...
def main():
    ...
    log(3, "Submitting compression task with video_codec='hevc', container='mkv', gpu_acceleration='auto'...")
    ...
    while time.time() - start_time < 30:
        st_req = urllib.request.urlopen(f"{BASE_URL}/api/compress/status/{task_id}", timeout=5)
    ...
    assert status == "completed", f"Task status expected 'completed', got '{status}': {st_json.get('error')}"
    ...
    log_req = urllib.request.urlopen(f"{BASE_URL}/api/compress/logs/{task_id}", timeout=5)
    ...
    log(5, "Log endpoint verified successfully!", f"Command logged: {log_json.get('cmd')}")

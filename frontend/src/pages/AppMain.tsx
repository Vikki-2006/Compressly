...
interface QueueItem {
  ...
  status: "uploading" | "ready" | "processing" | "paused" | "completed" | "cancelled" | "failed";
  metadata?: any;
  thumbnailUrl?: string | null;
  taskId?: string;
  progress: number; // compression 0-100
  elapsed: number;
  eta: number;
  speed: string;
...
export const AppMain: React.FC = () => {
  ...
  const pollingIntervals = useRef<{ [key: string]: any }>({});
  ...
  const uploadFile = (item: QueueItem) => {
  ...
  const startCompress = async (itemId: string) => {
  ...
  const startPolling = (itemId: string, taskId: string) => {
  ...
  const controlTask = async (itemId: string, action: "pause" | "resume" | "cancel") => {
    const item = queue.find((q) => q.id === itemId);
    if (!item) return;
  ...
  const downloadFile = (taskId: string, originalName: string) => {
  ...
  const downloadFromHistory = (taskId: string, filename: string) => {
    const link = document.createElement("a");
  ...
  const deleteHistoryRow = async (taskId: string) => {
    try {
...
Original Size: ${(meta.size / (1024 * 1024)).toFixed(2)}MB`;
  ...
  const getSavingsEstimate = (preset: string): string => {
